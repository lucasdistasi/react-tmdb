...
export const ShowInfoComponent = () => {
  ...
  return (
    _error ? <ErrorComponent/> : loading ? <SpinnerComponent/> : data && data.info ?
      <div>
        <div
          ...
          style={{
            backgroundImage: `url(${data.info.backdrop_path ?
              getPosterPath("original", data.info.backdrop_path) :
              NO_POSTER})`
          }}/>
        ...
        <div className="container mx-auto bg-gray-50">
          <div className="py-12">
            <div className="text-center">
              <h1 className="text-7xl py-6">{data.info.name}</h1>
              <TaglineComponent tagline={data.info.tagline}/>
              <TrailerComponent trailers={trailers} elementId={data.id} elementType="tv"/>
              <p className="text-lg mx-28 mt-12 pb-8">{data.info.overview}</p>
            </div>
            <div className="flex flex-col flex-wrap lg:flex-row items-center lg:justify-between mx-16 mt-10">
              <InfoIconComponent info={data.info.vote_average}
                                 icon={faStar}
                                 color="text-yellow-500"
                                 title="Rating"/>
              <InfoIconComponent info={data.info.first_air_date}
                                 icon={faCalendarAlt}
                                 color="text-green-500"
                                 title="First Air Date"/>
              <InfoIconComponent info={data.info.last_air_date}
                                 icon={faCalendarAlt}
                                 color="text-blue-500"
                                 title="Last Air Date"/>
              <InfoIconComponent info={data.info.number_of_seasons}
                                 icon={faVideo}
                                 color="text-indigo-500"
                                 title="Seasons"/>
              <InfoIconComponent info={data.info.number_of_episodes}
                                 icon={faFilm}
          ...
          <div className="py-14">
            <div className="flex flex-col lg:flex-row items-center lg:justify-around mx-16 mt-5">
              <GenreComponent genres={data.info.genres}/>
              <ProductionCompaniesComponent productionCompanies={data.info.production_companies}/>
              <StatusComponent status={data.info.status}/>
              <HomePageComponent homepage={data.info.homepage} title={data.info.name}/>
            </div>
          </div>

          <PeopleGridComponent
            people={data.info.created_by}
            title={"Created by"}
          ...
          <NetworkGridComponent networks={data.info.networks}
                                title={"Networks"}/>

          <NetworkGridComponent networks={data.info.production_companies}
                                title={"Production Companies"}/>

          {
            data.info.seasons && <SeasonAccordionComponent seasons={data.info.seasons} showId={data.id} originalPoster={data.info.backdrop_path}/>
          }
